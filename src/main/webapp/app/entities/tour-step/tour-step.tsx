  ...
  return (
    <div>
      <h2 id="tour-step-heading" data-cy="TourStepHeading">
        <Translate contentKey="citytoursApp.tourStep.home.title">Tour Steps</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="citytoursApp.tourStep.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/tour-step/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="citytoursApp.tourStep.home.createLabel">Create new Tour Step</Translate>
          </Link>
        </div>
      </h2>
        ...
        {tourStepList && tourStepList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="citytoursApp.tourStep.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                ...
                <th>
                  <Translate contentKey="citytoursApp.tourStep.tour">Tour</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th>
                  <Translate contentKey="citytoursApp.tourStep.place">Place</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {tourStepList.map((tourStep, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{tourStep.tour ? <Link to={`/tour/${tourStep.tour.id}`}>{tourStep.tour.title}</Link> : ''}</td>
                  <td>{tourStep.place ? <Link to={`/place/${tourStep.place.id}`}>{tourStep.place.name}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="citytoursApp.tourStep.home.notFound">No Tour Steps found</Translate>
            </div>
          )
        )}
      </div>
