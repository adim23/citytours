...
public interface TourStepRepository extends JpaRepository<TourStep, Long> {
    ...
    @Query(
        value = "select tourStep from TourStep tourStep left join fetch tourStep.tour left join fetch tourStep.place",
        countQuery = "select count(tourStep) from TourStep tourStep"
    )
    Page<TourStep> findAllWithToOneRelationships(Pageable pageable);

    @Query("select tourStep from TourStep tourStep left join fetch tourStep.tour left join fetch tourStep.place")
    List<TourStep> findAllWithToOneRelationships();

    @Query("select tourStep from TourStep tourStep left join fetch tourStep.tour left join fetch tourStep.place where tourStep.id =:id")
